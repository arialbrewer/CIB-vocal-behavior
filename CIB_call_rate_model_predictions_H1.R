...
newData1 <- as.data.frame(expand.grid(seq(1,53,1),c("Mill","Travel"),c("Flood"),c("no")))
...
for(i in 1:boots){
  ...
  yest[,i] <- predict(ymod,newdata = newData1, type="response", re.form=NA) + rbinom(1,1,sigma.obs)
}
...
#plot  #UPDATE WITH PROB OF CALLING
ggplot() +
  geom_line(data=preds, aes(x=exp(lgroup_size), y=mean, color=behavior), linewidth = 1) +
  geom_ribbon(data=preds, aes(x=exp(lgroup_size), ymin=conf.low, ymax=conf.high, fill=behavior), alpha = 0.05, color=NA) +
  geom_point(data=callrate_total_trunc, aes(x=group_size, y=n_minute, color=behavior), 
